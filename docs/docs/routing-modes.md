...
Customer-side DNS resolution and certificate-coverage requirements are documented in [Network Prerequisites](network_prerequisites). On the platform side, the typical procedure involves:

1. **Using cert-manager:** Deploy `cert-manager` to automatically provision and rotate certificates from Let's Encrypt or your internal CA.
2. **Updating the Istio Gateway secret:** Configure your Istio `Gateway` resource to reference the TLS secret generated by `cert-manager` (or manually provisioned) so that the ingress terminates TLS correctly.
