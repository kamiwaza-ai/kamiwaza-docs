...
- **Corporate DNS:** from end-user workstations, `<domain>` and `docs.<domain>` must resolve to the IP address where the Kamiwaza install is reachable. The Kamiwaza operator can provide this — for a typical single-host install, it is the install host's primary network IP.
- **TLS certificate:** issue a certificate covering `<domain>` and `docs.<domain>`, signed by a CA your end-user browsers already trust (typically your corporate CA), and deliver it to the Kamiwaza operator to install at the cluster edge.

A wildcard `*.<domain>` for both DNS and the certificate satisfies the above and covers any subdomains added by future platform features without further work. If wildcards are not permitted in your environment, the explicit names above are the current set; revisit on Kamiwaza upgrades.
