...
|---|---|---|
| **Offline** | Sized per the selected release handoff | Release-dependent — [compute it from the handoff](offline_install.md#check-the-actual-filesystems) |
...
df -h /var/lib
# Expected (online): 1.1TB+ at the default storage image size, or >=350GB when
# passing -e storage_host_prep_virtual_block_size=80G.
# Offline: the floor is release-dependent — compute it from the selected handoff
# per the offline guide's "Check the actual filesystems" step.

# Check the root filesystem too
df -h /
# Expected: At least 30GB free; for the offline install path size per the offline
# guide, since its scratch and copy locations vary by handoff.
