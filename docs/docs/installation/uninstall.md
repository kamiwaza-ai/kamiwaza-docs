...
> **Verify online uninstall completion.** A teardown gap was observed on released Linux online installs: the wrapper reported `Uninstallation Complete!` while k0s and the Kamiwaza pods were still running. Verify the cluster is gone as described in [Step 2](#step-2-run-the-matching-uninstall-wrapper). Use the matching release's wrapper first; the manual recovery below applies only if the Linux k0s cluster remains.

## What Gets Removed

A Kamiwaza install consists of:

- A single-host Kubernetes cluster and its container runtime, provisioned by the installer. Current supported single-host installs use k0s.
...
or a source-based developer install. Use the wrapper matching that install.
...
> The extracted payload is retained only when `--keep-extract` was used.
> If the matching wrapper is missing, identify the installer version and contact
> Kamiwaza support before removing directories. Do not substitute a wrapper
...
The recovery commands below apply to released **Linux** online installs running
k0s under systemd. They do not apply to source-based macOS developer installs,
which use the matching Lima-aware developer wrapper. Production macOS installs
are not currently supported (ENG-10839).
...
**For releases using Rook-Ceph loop-device storage, also reset the OSD
loop-device service (Linux only).** This legacy storage backend is managed by `kamiwaza-osd-loop.service`, a systemd oneshot unit. Once it has run
...
sudo rm -rf /etc/k0s
sudo rm -rf /var/lib/rook                     # only for releases using Rook-Ceph
```

> **For releases using Rook-Ceph, remove `/var/lib/rook` before reinstalling.** This
...
> **For releases using Rook-Ceph, do not retain stale OSD state when reinstalling.**
