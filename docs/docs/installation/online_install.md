...
> **Match your NVIDIA driver to the image's CUDA level.** The `-cuda12` packages
> require an NVIDIA driver of **550-server or later**; the `-cuda13` packages (and
> DGX Spark) require **580.65.06 or later**. These are the levels documented in the
> [GPU Drivers](system_requirements.md#gpu-drivers-required-for-gpu-inference) table —
> verify `nvidia-smi` against it before deploying a model. For the vLLM packages,
> install the driver that matches the CUDA level of the image you deploy; the DGX
> Spark ARM64 vLLM package uses the CUDA 13 driver level above.
