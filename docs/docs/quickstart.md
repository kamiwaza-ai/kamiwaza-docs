...
1. Open **Workrooms** from the navigation sidebar. Workrooms are built into Kamiwaza; do not deploy the deprecated Workroom Manager extension from App Garden. If **Workrooms** is not in the sidebar, the workrooms page is turned off for this installation; ask your administrator to enable it before running this check.
