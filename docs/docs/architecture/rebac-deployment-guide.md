...
1. Confirm only the new `AUTH_GATEWAY_*` block exists in `env.sh` by running `grep AUTH_GATEWAY env.sh`. `runtime/oidc-uat.env` should report the same values.
2. Generate a single ForwardAuth secret and let the rotation helper push it everywhere:
   ```bash
...
3. Rerun `./copy-compose.sh`, then bounce Traefik and the core stack:
   ...
   ```
4. Finally, verify all of the staged env files report the same secret:
   ```bash
   grep AUTH_FORWARD_HEADER_SECRET env.sh runtime/oidc-uat.env \
       deployment/envs/${KAMIWAZA_ENV:-default}/kamiwaza-traefik/*/.env
   ```

If any of these values drift, Traefik will forward unsigned headers and the UI will report `forward_auth_signature_invalid` during model downloads.
:::
...
`runtime/oidc-uat.env` is the source of truth for the Keycloak client metadata (issuer URL, JWKS path, client ID/secret). Never reintroduce older exports that point at different hosts or generate fresh secrets—those overrides are the most common root cause of `{"detail":"Authentication failed"}` when the gateway restarts.
...
Only use these flags in local or UAT environments. As soon as tokens carry the real tenant metadata, remove both exports so ReBAC enforces per-tenant policy.

### Seed ReBAC tuples

Before operators can download or deploy a model they must be granted the appropriate tuples in the target tenant. The quickest way to unblock labs is to seed the tuples that cover the default admin account:
...
2. Call the validation endpoint to ensure the session cookie works. Copy the `access_token` cookie from your browser (Developer Tools → Application → Cookies → `access_token`) and pass it in the request. If you are still using a self-signed certificate in a lab, include `-k` so curl trusts it.
   ```bash
...
3. If you receive a redirect loop, `401`, or `{"detail":"Authentication failed"}`, confirm `AUTH_GATEWAY_COOKIE_DOMAIN` matches the hostname and that every Traefik router attaches the `forwardauth@file` middleware. You can spot-check the staged bundles with:
   ```bash
   rg -n "forwardauth" deployment/kamiwaza-traefik
   ```
   The `forwardAuth` block must contain `authRequestHeaders` (`Cookie`, `Authorization`) and the standard `authResponseHeaders` (`X-User-*` plus the signature fields). After editing, run `./copy-compose.sh`, `sudo ./containers-down.sh traefik`, `sudo ./containers-up.sh traefik`, and `./startup/kamiwazad.sh restart-core` so the changes take effect.
4. If the signature still fails, resync the environment exports and restart the services:
   ```bash
   source env.sh
   source runtime/oidc-uat.env
