...
2. **Edit documentation**
   - Edit files in `docs/docs/` for platform docs
   - SDK docs are synced from the `kamiwaza-sdk` repo — edit them there, then run `npm run sync-sdk`
...
├── sdk/            # SDK documentation (synced from kamiwaza-sdk repo, don't edit directly)
│   ├── services/   # Service docs (gitignored, generated by sync-sdk)
│   └── index.md    # Landing page (gitignored, generated by sync-sdk)
├── blog/           # Blog posts
...
### SDK Sync
```bash
# Auto-detects kamiwaza-sdk as sibling directory
npm run sync-sdk
