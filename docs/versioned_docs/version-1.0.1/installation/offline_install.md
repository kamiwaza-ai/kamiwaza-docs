...
The extension-bundle filename is release-specific; take the exact name from the Keygen 1.0.1 release listing and set `EXT_BUNDLE` accordingly.

```bash
export KEYGEN_TOKEN="<license-key>"
export RELEASE="1.0.1"
export EXT_BUNDLE="<kamiwaza-extensions-bundle-...tar.gz>"   # exact name from the 1.0.1 release
