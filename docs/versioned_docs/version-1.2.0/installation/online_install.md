...
Choose and record an explicit supported version. Do not use an unrecorded
`latest` alias for a production install. The example defaults to 1.2.0 but can
be pinned by the release owner:

```bash
KAMIWAZA_VERSION="${KAMIWAZA_VERSION:-1.2.0}"
...
> installer selected above must match the exact candidate approved by the
> upgrade runbook.
