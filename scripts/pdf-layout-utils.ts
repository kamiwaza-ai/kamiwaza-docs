 ...
 */
export function inferTocSectionFromDocId(docId: string): string | undefined {
...
export interface TocDocument {
	/** Docs route id (empty string for site intro home). Used for section inference. */
	id: string;
	title: string;
	url: string;
	/** Optional: explicit nav section (from Docusaurus sidebar category label or pdf-config). */
	section?: string;
}

export interface TocHtmlOptions {
	title: string;
	subtitle: string;
	versionLabel?: string;
	logoBase64?: string;
	/** When set (one per document row), printed after the dotted leader in the TOC */
...
function tocPageSpan(
	entryPageNumbers: number[] | undefined,
	docIndex: number,
): string {
...
export function buildTocHtml({
	...
	entryPageNumbers,
	documents,
}: TocHtmlOptions): string {
	let docIndex = 0;
	let primaryNum = 0;
	const tocItemParts: string[] = [];
...
function groupDocumentsForToc(documents: TocDocument[]): {
	section: string | undefined;
	docs: TocDocument[];
}[] {
	const groups: { section: string | undefined; docs: TocDocument[] }[] = [];
	for (const doc of documents) {
		const effective =
			doc.section?.trim() || inferTocSectionFromDocId(doc.id);
