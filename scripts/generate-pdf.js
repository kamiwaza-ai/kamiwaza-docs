...
class PDFGenerator {
    ...
    safePath(...segments) {
        ...
        if (!resolved.startsWith(this.projectRoot + path.sep) &&
            ...
            throw new Error(`Path traversal blocked: ${resolved} is outside ${this.projectRoot}`);
        }
        return resolved;
    }
    constructor(configPath) {
        ...
        if (!resolvedConfigPath.startsWith(this.projectRoot + path.sep) &&
            resolvedConfigPath !== this.projectRoot) {
            throw new Error(`Config path must be within project root: ${this.projectRoot}`);
        }
    ...
    async generate(profileName, version) {
    ...
    async discoverAllDocuments(excludeDocs, version) {
    ...
    async startServer() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.server = (0, child_process_1.spawn)("npm", ["run", "serve", "--", "--port", portStr], {
                cwd: docsPath,
                shell: true,
                stdio: "pipe",
            });
