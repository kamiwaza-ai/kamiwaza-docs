...
const TARGET_DIR = path_1.default.resolve(__dirname, "../docs/api");
const TARGET_FILE = path_1.default.join(TARGET_DIR, OPENAPI_FILENAME);
function resolveKamiwazaRepoPath() {
    const override = process.env.KAMIWAZA_REPO_PATH;
    const candidates = [
        override,
        path_1.default.resolve(__dirname, "../../platform/kamiwaza"),
        path_1.default.resolve(__dirname, "../../../platform/kamiwaza"),
        path_1.default.resolve(__dirname, "../../kamiwaza"),
    ].filter(Boolean);
    for (const candidate of candidates) {
        if (fs_extra_1.default.existsSync(path_1.default.join(candidate, ".git"))) {
            return candidate;
        }
    }
    const tried = candidates.map((c) => `  - ${c}`).join("\n");
    throw new Error(`Unable to locate kamiwaza repo. Set KAMIWAZA_REPO_PATH or place the repo in one of the expected locations:\n${tried}`);
}
...
async function fetchFromRunningPlatform() {
    const apiUrl = process.env.KAMIWAZA_API_URL || "https://kamiwaza.test/api/openapi.json";
    console.log(`Fetching OpenAPI spec from running platform: ${apiUrl}`);
    try {
        const response = await fetch(apiUrl);
        if (!response.ok) {
            throw new Error(`Failed to fetch from ${apiUrl}: ${response.status} ${response.statusText}`);
        }
...
function getOpenAPIFromLocalRepo(repoPath) {
    const openapiPath = path_1.default.join(repoPath, OPENAPI_FILENAME);
    ...
    return fs_extra_1.default.readFileSync(openapiPath, "utf-8");
}
function generateOpenAPIFromLocalSource(repoPath) {
    const branch = getCurrentBranch(repoPath);
    console.log(`Generating OpenAPI spec from local source: ${repoPath}`);
    console.log(`  Current branch: ${branch}`);
    const tempDir = fs_extra_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), "kamiwaza-openapi-"));
    const tempRoot = path_1.default.join(tempDir, "root");
    fs_extra_1.default.ensureDirSync(tempRoot);
    const pythonScript = `
import json
...
`.trim();
    try {
        return (0, child_process_1.execFileSync)("./scripts/kw_py", ["-c", pythonScript], {
            cwd: repoPath,
            encoding: "utf-8",
            stdio: ["pipe", "pipe", "pipe"],
            env: {
                ...
                DATABASE_URL: process.env.DATABASE_URL || `sqlite:///${path_1.default.join(tempDir, "main.db")}`,
                CLUSTER_DATABASE_URL: process.env.CLUSTER_DATABASE_URL ||
                    `sqlite:///${path_1.default.join(tempDir, "cluster.db")}`,
                AUTH_DATABASE_URL: process.env.AUTH_DATABASE_URL || `sqlite:///${path_1.default.join(tempDir, "auth.db")}`,
                AUTH_FORWARD_HEADER_SECRET: process.env.AUTH_FORWARD_HEADER_SECRET ||
...
async function resolveOpenAPIContent(repoPath, generateFromPlatform) {
...
function getDocsVersion() {
    const packageJsonPaths = [
        ...
        path_1.default.resolve(__dirname, "../docs/package.json"),
    ];
    ...
    console.warn("Unable to determine docs version from package.json; defaulting API docs version to 0.0.0.");
    return "0.0.0";
}
function patchSpecForDocs(spec) {
    ...
    const chunkResponse = spec?.paths?.["/embedding/chunk"]?.post?.responses?.["200"];
...
async function main() {
    ...
    await fs_extra_1.default.ensureDir(TARGET_DIR);
    // Write the spec
    await fs_extra_1.default.writeFile(TARGET_FILE, JSON.stringify(spec, null, 2));
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(TARGET_DIR, "openapi-metadata.json"), JSON.stringify(metadata, null, 2));
    console.log("\nSync complete!");
    console.log("\nNext steps:");
